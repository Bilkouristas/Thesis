  ...
  public void onDrawFrame(SampleRender render){
    ...
    if(buttonClicked) {
      try {
        ...
        int N = depthImage.getWidth();
        int M = depthImage.getHeight();
        ...
        for (int y = 0; y < depthImage.getHeight(); y++) {
          for (int x = 0; x < depthImage.getWidth(); x++) {
                ...
                result[x][y] = getMillimetersDepth(depthImage, x, y);
        ...
        //Image snapshot = frame.acquireDepthImage();
        ...
        runOnUiThread(new Runnable() {
          ...
          public void run() {
            ...
            /////////////////////////////////////////////
            try {
              Bitmap myBitmap = yuv420ToBitmap(frame.acquireCameraImage());
              imageView.setImageBitmap(myBitmap);
            } catch (NotYetAvailableException e) {
              e.printStackTrace();
            }
            /////////////////////////////////////////
            //Bitmap myBitmap = bitmapFromArray(dep);

//            ByteBuffer buffer = snapshot.getPlanes()[0].getBuffer();
...
//            Bitmap myBitmap = BitmapFactory.decodeByteArray(bytes,0,bytes.length,null);;
        ...
        Log.d("RESULT", "VALUE: " + Arrays.deepToString(result));
...
//          for (int x = 0; x < depthImage.getWidth(); x++) {
//
//            int result = getMillimetersDepth(depthImage, x, y);
